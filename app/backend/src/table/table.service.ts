...
export class TableService{
    ...
    async findByRestaurantAndTableNumber(restaurantId: number, tableNo: number): Promise<any> {
        ...
        const table = await this.tableRepository.findOne({ where: { restaurant: { id: restaurantId }, table_no: tableNo }});
        if (!table) {
    ...
    async create(createTableDto: CreateTableDto): Promise<createTableResponseDto> {
        ...
        const existingTable = await this.tableRepository.findOne({ where: { restaurant: { id: restaurant_id }, table_no }});
        if (existingTable) {
